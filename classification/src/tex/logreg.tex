...
f(X, \hat{\theta}),
\end{equation*}
где $f$ --- вид функции, которой приближаем зависимость $f^{\ast}$, $\hat{\theta}$ --- вектор параметров оценки, $\hat{Y}$ --- предсказанное значение $Y$.

Когда целью является предсказание, нам не важна точная форма функции $f(X, \hat{\theta})$, если она обеспечивает точные предсказания $Y$.
...
Пусть имеется выборка из $n$ отдельных наблюдений: $x_{1},\ldots,x_{n}$ для которых известны соответствующие значения отклика. Обозначим $x_{ij}$ --- значение $j$-го признака $i$-го наблюдения, $x_{i}=(x_{i1},\ldots,x_{ip})\tr$,  $y_{i}$ --- отклик у $i$-го наблюдения.
 ...
 Хотим найти такую функцию $f$, что $y\approx f(x)$ для любого наблюдения $(x,y)$.  
...
Пусть $f_j:X\to\mathbb{R},~j=1,\ldots,p$. Линейная модель классификации: 
 \begin{equation*}
 f(x,\theta)=\sign (\theta_0 + \sum\limits_{j=1}^p\theta_jf_j(x)).
 \end{equation*}

Если добавить к признакам единицу и положить $f_j(x) = x$, получаем
\[
...
Для оценки качества предсказания необходимо ввести функционал качества. Для этого сначала введём отступ и функцию потерь.

\begin{definition}
Отступ $\mathsf{M=\langle \theta,x \rangle y}$ --- <<расстояние>> между реальным значением и предсказанным.
\end{definition}

\begin{definition}
	Функция потерь (loss function) $L(M)$ --- неотрицательная функция, характеризующая величину ошибки предсказания на объекте $x$. 
\end{definition}

Для задачи классификации в качестве функции потерь обычно используется индикатор ошибки $L(M) = \sign(M)$.

Теперь определим эмпирический риск --- функционал, который используется для оценки качества предсказания.

\begin{definition}
	Эмпирический риск $Q(X_n, \theta) = \displaystyle\frac{1}{n}\displaystyle\sum_{i=1}^{n} L(M_i)$
\end{definition}
...
Пусть $f(x,\hat{\theta})=\sign (\theta_1f_1(x)+\theta_2f_2(x)-\theta_0)$, $f_2(x)=kf_1(x)$. \\ Тогда $\theta_1f_1(x)+\theta_2f_2(x)=(\theta_1+\beta)f_1(x)+(\theta_2-k\beta)f_2(x) \ \forall \beta$
\end{itemize}
Таким образом, у оценки параметров большая дисперсия. Для уменьшения дисперсии можно добавить небольшое смещение, используя регуляризацию. \\ 
